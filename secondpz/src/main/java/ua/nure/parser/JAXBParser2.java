...
import ua.nure.film.entity.article.Articles;
...
public class JAXBParser2 {
	...
	public static void main(String[] args) throws JAXBException, FileNotFoundException, SAXException {
		...
		Film film = films.getFilm().get(0);
		Marshaller marshaller = jc.createMarshaller();

		// It allows to validate resulting document
//		SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
		
		// It will throw an exception if there is no "article" element were declared in article.xsd
//		marshaller.setSchema(schemaFactory.newSchema(new File("article.xsd")));
//		marshaller.setProperty(null, schemaFactory);
		
		// serialization the object of class that is not annotated @XmlRootElement
		JAXBElement<Film> jarticle = new JAXBElement<>(
				new QName(Const.FILMS_NAMESPACE_URI, "film"),
				Film.class, film);
		
		marshaller.marshal(jarticle, new File("oneFilm.xml"));
	}
}